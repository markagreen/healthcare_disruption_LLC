...
# Create table
table1 <- data.frame(svymean(sensitivity_analyses$time_to_ambulatory_all, na.rm=T, wgt3)) # Save frequency mean

hold <- data.frame(svymean(sensitivity_analyses$`Time to Ambulatory Care Sensitive Acute`, na.rm=T, wgt3)) # Repeat for each outcome
table1 <- cbind(table1, hold)

hold <- data.frame(svymean(sensitivity_analyses$`Time to Ambulatory Care Sensitive Chronic`, na.rm=T, wgt3))
table1 <- cbind(table1, hold) 

hold <- data.frame(svymean(sensitivity_analyses$`Time to Ambulatory Care Sensitive Vaccine-preventable`, na.rm=T, wgt3))
table1 <- cbind(table1, hold) 

hold <- data.frame(svymean(sensitivity_analyses$`Time to Emergency Urgent Care Sensitive `, na.rm=T, wgt3))
table1 <- cbind(table1, hold)

hold <- data.frame(svymean(sensitivity_analyses$time_to_total_admissions, na.rm=T, wgt3))
table1 <- cbind(table1, hold) 
...
model_surv_any_a_u <- coxph(Surv(time_to_ambulatory_all, censor) ~ imp_disruption_any, data = sensitivity_analyses) # Unadjusted
table2 <- tidy(model_surv_any_a_u, conf.int = TRUE) # Save key results
table2 <- table2[1,] # Extract only healthcare disruption variable
table2$model <- "All ACS - unadj" # Note model name

model_surv_any_a_a <- coxph(Surv(time_to_ambulatory_all, censor) ~ imp_disruption_any + factor(imp_sex) + imp_age + imp_age2 + factor(imp_not_white) + factor(imp_own_home) + factor(imp_general_health) + factor(imp_imd2019_income_q5) + factor(cohort) + imp_quan_cci, data = sensitivity_analyses) # Adjusted
hold <- tidy(model_surv_any_a_a, conf.int = TRUE) # Save key results
hold <- hold[1,] # Extract only healthcare disruption variable
...
model_surv_any_b_u <- coxph(Surv(`Time to Ambulatory Care Sensitive Acute`, censor) ~ imp_disruption_any, data = sensitivity_analyses) # Unadjusted
hold <- tidy(model_surv_any_b_u, conf.int = TRUE) # Save key results
hold <- hold[1,] # Extract only healthcare disruption variable
hold$model <- "Acute ACS - unadj" # Note model name
table2 <- rbind(table2, hold) # Join to main table

model_surv_any_b_a <- coxph(Surv(`Time to Ambulatory Care Sensitive Acute`, censor) ~ imp_disruption_any + factor(imp_sex) + imp_age + imp_age2 + factor(imp_not_white) + factor(imp_own_home) + factor(imp_general_health) + factor(imp_imd2019_income_q5) + factor(cohort) + imp_quan_cci, data = sensitivity_analyses) # Adjusted
...
model_surv_any_c_u <- coxph(Surv(`Time to Ambulatory Care Sensitive Chronic`, censor) ~ imp_disruption_any, data = sensitivity_analyses) # Unadjusted
hold <- tidy(model_surv_any_c_u, conf.int = TRUE) # Save key results
hold <- hold[1,] # Extract only healthcare disruption variable
hold$model <- "Chronic ACS - unadj" # Note model name
table2 <- rbind(table2, hold) # Join to main table

model_surv_any_c_a <- coxph(Surv(`Time to Ambulatory Care Sensitive Chronic`, censor) ~ imp_disruption_any + factor(imp_sex) + imp_age + imp_age2 + factor(imp_not_white) + factor(imp_own_home) + factor(imp_general_health) + factor(imp_imd2019_income_q5) + factor(cohort) + imp_quan_cci, data = sensitivity_analyses) # Adjusted
...
model_surv_any_d_u <- coxph(Surv(`Time to Ambulatory Care Sensitive Vaccine-preventable`, censor) ~ imp_disruption_any, data = sensitivity_analyses) # Unadjusted
hold <- tidy(model_surv_any_d_u, conf.int = TRUE) # Save key results
hold <- hold[1,] # Extract only healthcare disruption variable
hold$model <- "Vaccine preventable ACS - unadj" # Note model name
table2 <- rbind(table2, hold) # Join to main table

model_surv_any_d_a <- coxph(Surv(`Time to Ambulatory Care Sensitive Vaccine-preventable`, censor) ~ imp_disruption_any + factor(imp_sex) + imp_age + imp_age2 + factor(imp_not_white) + factor(imp_own_home) + factor(imp_general_health) + factor(imp_imd2019_income_q5) + factor(cohort) + imp_quan_cci, data = sensitivity_analyses) # Adjusted
...
model_surv_any_e_u <- coxph(Surv(`Time to Emergency Urgent Care Sensitive `, censor) ~ imp_disruption_any, data = sensitivity_analyses) # Unadjusted
hold <- tidy(model_surv_any_e_u, conf.int = TRUE) # Save key results
hold <- hold[1,] # Extract only healthcare disruption variable
hold$model <- "EUCS - unadj" # Note model name
table2 <- rbind(table2, hold) # Join to main table

model_surv_any_e_a <- coxph(Surv(`Time to Emergency Urgent Care Sensitive `, censor) ~ imp_disruption_any + factor(imp_sex) + imp_age + imp_age2 + factor(imp_not_white) + factor(imp_own_home) + factor(imp_general_health) + factor(imp_imd2019_income_q5) + factor(cohort) + imp_quan_cci, data = sensitivity_analyses) # Adjusted
...
model_surv_any_f_u <- coxph(Surv(time_to_total_admissions, censor) ~ imp_disruption_any, data = sensitivity_analyses) # Unadjusted
hold <- tidy(model_surv_any_f_u, conf.int = TRUE) # Save key results
hold <- hold[1,] # Extract only healthcare disruption variable
hold$model <- "Any adm - unadj" # Note model name
table2 <- rbind(table2, hold) # Join to main table

model_surv_any_f_a <- coxph(Surv(time_to_total_admissions, censor) ~ imp_disruption_any + factor(imp_sex) + imp_age + imp_age2 + factor(imp_not_white) + factor(imp_own_home) + factor(imp_general_health) + factor(imp_imd2019_income_q5) + factor(cohort) + imp_quan_cci, data = sensitivity_analyses) # Adjusted
...
# Model a: Any ambulatory care sensitive conditions
model_surv_type_a_u <- coxph(Surv(time_to_ambulatory_all, censor) ~ imp_disruption_appointments + imp_disruption_medications + imp_disruption_procedures, data = sensitivity_analyses) # Unadjusted
table3 <- tidy(model_surv_type_a_u, conf.int = TRUE) # Save key results
table3 <- table3[1:3,] # Extract only healthcare disruption variable
table3$model <- "All ACS - unadj" # Note model name

model_surv_type_a_a <- coxph(Surv(time_to_ambulatory_all, censor) ~ imp_disruption_appointments + imp_disruption_medications + imp_disruption_procedures + factor(imp_sex) + imp_age + imp_age2 + factor(imp_not_white) + factor(imp_own_home) + factor(imp_general_health) + factor(imp_imd2019_income_q5) + factor(cohort) + imp_quan_cci, data = sensitivity_analyses) # Adjusted
hold <- tidy(model_surv_type_a_a, conf.int = TRUE) # Save key results
hold <- hold[1:3,] # Extract only healthcare disruption variable
hold$model <- "All ACS - adj" # Note model name
table3 <- rbind(table3, hold) # Join to main table

# Model b: Acute ambulatory care sensitive acute conditions
model_surv_type_b_u <- coxph(Surv(`Time to Ambulatory Care Sensitive Acute`, censor) ~ imp_disruption_appointments + imp_disruption_medications + imp_disruption_procedures, data = sensitivity_analyses) # Unadjusted
hold <- tidy(model_surv_type_b_u, conf.int = TRUE) # Save key results
hold <- hold[1:3,] # Extract only healthcare disruption variable
hold$model <- "Acute ACS - unadj" # Note model name
table3 <- rbind(table3, hold) # Join to main table

model_surv_type_b_a <- coxph(Surv(`Time to Ambulatory Care Sensitive Acute`, censor) ~ imp_disruption_appointments + imp_disruption_medications + imp_disruption_procedures + factor(imp_sex) + imp_age + imp_age2 + factor(imp_not_white) + factor(imp_own_home) + factor(imp_general_health) + factor(imp_imd2019_income_q5) + factor(cohort) + imp_quan_cci, data = sensitivity_analyses) # Adjusted
...
model_surv_type_c_u <- coxph(Surv(`Time to Ambulatory Care Sensitive Chronic`, censor)  ~ imp_disruption_appointments + imp_disruption_medications + imp_disruption_procedures, data = sensitivity_analyses) # Unadjusted
hold <- tidy(model_surv_type_c_u, conf.int = TRUE) # Save key results
hold <- hold[1:3,] # Extract only healthcare disruption variable
hold$model <- "Chronic ACS - unadj" # Note model name
table3 <- rbind(table3, hold) # Join to main table

model_surv_type_c_a <- coxph(Surv(`Time to Ambulatory Care Sensitive Chronic`, censor)  ~ imp_disruption_appointments + imp_disruption_medications + imp_disruption_procedures + factor(imp_sex) + imp_age + imp_age2 + factor(imp_not_white) + factor(imp_own_home) + factor(imp_general_health) + factor(imp_imd2019_income_q5) + factor(cohort) + imp_quan_cci, data = sensitivity_analyses) # Adjusted
...
model_surv_type_d_u <- coxph(Surv(`Time to Ambulatory Care Sensitive Vaccine-preventable`, censor) ~ imp_disruption_appointments + imp_disruption_medications + imp_disruption_procedures, data = sensitivity_analyses) # Unadjusted
hold <- tidy(model_surv_type_d_u, conf.int = TRUE) # Save key results
hold <- hold[1:3,] # Extract only healthcare disruption variable
hold$model <- "Vaccine preventable ACS - unadj" # Note model name
table3 <- rbind(table3, hold) # Join to main table

model_surv_type_d_a <- coxph(Surv(`Time to Ambulatory Care Sensitive Vaccine-preventable`, censor) ~ imp_disruption_appointments + imp_disruption_medications + imp_disruption_procedures + factor(imp_sex) + imp_age + imp_age2 + factor(imp_not_white) + factor(imp_own_home) + factor(imp_general_health) + factor(imp_imd2019_income_q5) + factor(cohort) + imp_quan_cci, data = sensitivity_analyses) # Adjusted
...
# Model e: Emergency Urgent Sensitive conditions
model_surv_type_e_u <- coxph(Surv(`Time to Emergency Urgent Care Sensitive `, censor) ~ imp_disruption_appointments + imp_disruption_medications + imp_disruption_procedures, data = sensitivity_analyses) # Unadjusted
hold <- tidy(model_surv_type_e_u, conf.int = TRUE) # Save key results
hold <- hold[1:3,] # Extract only healthcare disruption variable
hold$model <- "EUCS - unadj" # Note model name
table3 <- rbind(table3, hold) # Join to main table

model_surv_type_e_a <- coxph(Surv(`Time to Emergency Urgent Care Sensitive `, censor) ~ imp_disruption_appointments + imp_disruption_medications + imp_disruption_procedures + factor(imp_sex) + imp_age + imp_age2 + factor(imp_not_white) + factor(imp_own_home) + factor(imp_general_health) + factor(imp_imd2019_income_q5) + factor(cohort) + imp_quan_cci, data = sensitivity_analyses) # Adjusted
...
# Model f: Any hospital admission
model_surv_type_f_u <- coxph(Surv(time_to_total_admissions, censor) ~ imp_disruption_appointments + imp_disruption_medications + imp_disruption_procedures, data = sensitivity_analyses) # Unadjusted
hold <- tidy(model_surv_type_f_u, conf.int = TRUE) # Save key results
hold <- hold[1:3,] # Extract only healthcare disruption variable
hold$model <- "Any adm - unadj" # Note model name
table3 <- rbind(table3, hold) # Join to main table

model_surv_type_f_a <- coxph(Surv(time_to_total_admissions, censor) ~ imp_disruption_appointments + imp_disruption_medications + imp_disruption_procedures + factor(imp_sex) + imp_age + imp_age2 + factor(imp_not_white) + factor(imp_own_home) + factor(imp_general_health) + factor(imp_imd2019_income_q5) + factor(cohort) + imp_quan_cci, data = sensitivity_analyses) # Adjusted
